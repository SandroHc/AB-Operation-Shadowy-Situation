...
namespace Kender.uGUI {
	...
	public class ComboBox : MonoBehaviour {
		...
		private RectTransform scrollPanelRectTransfrom {
			get {
				if(_scrollPanelRectTransfrom == null) _scrollPanelRectTransfrom = overlayGO.transform.Find("ScrollPanel").GetComponent<RectTransform>();
		...
		private RectTransform itemsRectTransfrom {
			get {
				if(_itemsRectTransfrom == null) _itemsRectTransfrom = scrollPanelRectTransfrom.Find("Items").GetComponent<RectTransform>();
		...
		private void InitControl() {
			...
			scrollPanelGO.transform.localScale = Vector2.one;
		...
		private void ToggleComboBox(bool directClick) {
			...
			if(overlayGO.activeSelf) {
				var buttonHeight = buttonRectTransform.sizeDelta.y;
				var comboYPosition = buttonRectTransform.position.y;
				var itemsToDisplay = Mathf.Min(_itemsToDisplay, Items.Length - (HideFirstItem ? 1 : 0));
				var yPos = comboYPosition < buttonHeight * (itemsToDisplay + 0.5f) ? comboYPosition + buttonHeight * 0.5f : comboYPosition - buttonHeight * (itemsToDisplay + 0.5f);
