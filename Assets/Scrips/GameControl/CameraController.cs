...
public class CameraController : MonoBehaviour {
	...
	void Update() {
		...
		transform.localPosition = new Vector3(
				Mathf.Sin(headbobStepCounter) * headbobAmountX * currentAimRacio,
				(Mathf.Cos(headbobStepCounter * 2) * headbobAmountY * -1 * currentAimRacio) + (transform.parent.localScale.y * eyeHeightRacio) - (transform.parent.localScale.y / 2),
